...
app.post('/cards', (req, res) => {
    ...
    const { titulo, conteudo, date, lista, id } = req.body;
    if (titulo && conteudo && date && lista && !id) {
        const card = { titulo, conteudo, date, lista, id: uuid() };
        cards.push(card);
...
const validateAndLogAlterationOrDeletion = (req, res, next) => {
    ...
    if (req.method === 'PUT') {
        ...
        if (!(titulo && conteudo && date && lista && id)) return res.sendStatus(400);
